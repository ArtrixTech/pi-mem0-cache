...
- **Embedding recall** (`src/index.ts` → `createJinaEmbedder`, `ensureEmbeddings`, `searchLocalVector`, `createEmbedHarness`): when `JINA_API_KEY` is set, incrementally embeds the memory corpus into `~/.pi/agent/mem0-vectors.json` (sha1-hash-tracked, model-tagged) and ranks gated/fallback searches by cosine similarity, degrading to keyword overlap on provider failure (1-minute cooldown).
...
All logic lives in the single dependency-free module `src/index.ts`; the extension entry captures the *unwrapped* fetch for the sync runner so replayed writes never re-enter the interceptor. Tests in `test/interceptor.test.ts`, `test/shadow.test.ts`, `test/shadow-entry.test.ts`, `test/embed.test.ts`, and `test/embed-entry.test.ts` exercise everything below (and including) the entry point.
